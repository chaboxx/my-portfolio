  ...
  return (
    <div className={styles.proyects_main_container}>
      <Modal isOpen={isOpen} toggleOpen={toggleOpen} position="center"> 
        ...
        <Image loading="eager" quality={100} src={currentImageModal} width={`${currentWidth > 760 ? currentWidth*0.6 : currentWidth*0.9}px`} height={`${currentWidth >760 ? currentWidth*0.4 : currentWidth * 0.8}px`} layout="fixed"/>
          ...
          proyectos.map(data=>(
            <div key={data.id} className={styles.content_item}>
              <div onClick={(e)=>openModalImage(e,data.image)} className={styles.image_container}>
                <Image loading="eager" quality={100} layout="responsive" src={data.image} width="100%" height="100%"/>
              </div>
